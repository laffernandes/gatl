...
#include "common.hpp"
#include <gatl/ga1e.hpp>

using namespace ga1e;

constexpr ga::ndims_t N = space_t::vector_space_dimensions;
constexpr std::size_t SIZE = 1 << N;

auto prod = [](auto const &lhs, auto const &rhs) { return sp(lhs, rhs); };
auto truth = [](auto const &lhs, auto const &rhs) { return take_grade(gp(lhs, rhs), 0); };
