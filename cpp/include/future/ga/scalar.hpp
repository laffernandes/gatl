...
namespace ga {
	...
	constexpr decltype(auto) scalar(ValueType const &arg) {
		return clifford_expression<ValueType, detail::value<detail::stored> >(make_sequential_storage(arg));
	}

	template<class ValueType, class = std::enable_if_t<!is_clifford_expression_v<ValueType> > >
	constexpr decltype(auto) scalar(ValueType &&arg) {
		return clifford_expression<ValueType, detail::value<detail::stored> >(make_sequential_storage(std::move(arg)));
	}
