...
#ifndef __GA_MODEL_SIGNED_MINKOWSKI_EUCLIDEAN_VECTOR_HPP__
#define __GA_MODEL_SIGNED_MINKOWSKI_EUCLIDEAN_VECTOR_HPP__
...
namespace ga {
    ...
    constexpr decltype(auto) euclidean_vector(minkowski_metric_space<D> const &mtr, Types &&... coords) GA_NOEXCEPT {
    ...
    constexpr decltype(auto) euclidean_vector(minkowski_metric_space<D> const &mtr, IteratorType begin, IteratorType end) GA_NOEXCEPT {
        assert(D == std::distance(begin, end));
        return detail::make_vector_using_iterator(mtr, begin, std::make_index_sequence<D>{}, c<0>, c<0>);
    }

}

#endif // __GA_MODEL_SIGNED_MINKOWSKI_EUCLIDEAN_VECTOR_HPP__
