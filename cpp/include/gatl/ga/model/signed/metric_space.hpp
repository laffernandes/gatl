...
namespace ga {
    ...
    public:
        ...
        constexpr static bitset_t basis_vectors = bitset_t(bitset_t(~0) >> (std::numeric_limits<bitset_t>::digits - (P + Q + R)));
        constexpr static ndims_t vector_space_dimensions = P + Q + R;

    private:

        constexpr static bitset_t negative = (Q != 0) ? bitset_t(bitset_t(~0) >> (std::numeric_limits<bitset_t>::digits - Q)) << P : bitset_t(0);
        constexpr static bitset_t zero = (R != 0) ? bitset_t(bitset_t(~0) >> (std::numeric_limits<bitset_t>::digits - R)) << (P + Q) : bitset_t(0);
