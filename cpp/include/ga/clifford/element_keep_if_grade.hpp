#ifndef __GA_CLIFFORD_ELEMENT_KEEP_IF_GRADE_HPP__
#define __GA_CLIFFORD_ELEMENT_KEEP_IF_GRADE_HPP__
...
namespace ga {
	...
	namespace clifford {
		...
		namespace detail {
			...
			template<grade_t K>
			class keep_if_grade_func<constant<K> > {
			public:
			...
			struct _keep_if_grade_components_maybe_keep<default_bitset_t(1)> {
				...
				constexpr static decltype(auto) bind(components<CoefficientType, PossibleGrades> const &arg, KeepIfGradeFunc const &keep) {
					...
					if (itr != arg.end()) {
						...
						return make_component(CoefficientType(), cbasis_blade<0>());
					}
				}
			};
...
#endif // __GA_CLIFFORD_ELEMENT_KEEP_IF_GRADE_HPP__
