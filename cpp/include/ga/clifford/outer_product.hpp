...
namespace ga {
	...
	namespace clifford {
		...
		constexpr decltype(auto) op(LeftType const &lhs, RightType const &rhs, metric_space<MetricSpaceType> const &mtr) {
			return detail::try_cast_to_native(detail::graded_product(detail::begin(lhs), detail::begin(rhs), mtr, detail::op_func()));
		}

		template<class... LeftElementTypes, class... RightElementTypes, typename std::enable_if<detail::may_cast_to_native<clifford_expression<LeftElementTypes...> >::value || detail::may_cast_to_native<clifford_expression<RightElementTypes...> >::value, int>::type = 0>
		...
		constexpr decltype(auto) operator^(clifford_expression<LeftElementTypes...> const &lhs, RightType const &rhs) {
			return op(lhs, val(rhs), euclidean_metric_space<GA_MAX_BASIS_VECTOR_INDEX>());
		}
		...
		constexpr decltype(auto) operator^(LeftType const &lhs, clifford_expression<RightElementTypes...> const &rhs) {
			return op(val(lhs), rhs, euclidean_metric_space<GA_MAX_BASIS_VECTOR_INDEX>());
		}
