#ifndef __GA_SIGNED_METRIC_SPACE_HPP__
#define __GA_SIGNED_METRIC_SPACE_HPP__
...
namespace ga {
	...
	public:
		...
		constexpr static detail::cvalue<1> metric_factor(default_bitset_t const arg) {
			assert(includes_basis_blade(arg));
			return detail::cvalue<1>();
		}

		static_assert(P <= GA_MAX_BASIS_VECTOR_INDEX, "ga::signed_metric_space<P, 0> is ill-defined. It is expectated 0 <= P <= GA_MAX_BASIS_VECTOR_INDEX.");
	};
	...
	namespace detail {
		...
		struct metric_space_traits<signed_metric_space<P, Q> > {
			...
			struct cmetric_entry {
				...
				constexpr static auto value = cvalue<Row == Col ? (Row <= P ? 1 : -1) : 0>();
			};
...
#endif // __GA_SIGNED_METRIC_SPACE_HPP__
