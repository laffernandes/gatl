...
#ifndef _GA_HOMOGENEOUS_POINT_
#define _GA_HOMOGENEOUS_POINT_
...
point(const real_t *coordinates)
{
	...
	_private::point<typename multivector_type::store_t,multivector_type::model_t::origin_vector_bitmap>::run( result.data(), coordinates );
	return result;
}
...
point(const real_t &coordinate1, ...)
{
	...
	static const size_t size = store_t::size - ((ga::_private::is_stored<model_t::origin_vector_bitmap,store_t>::result) ? 1 : 0);
