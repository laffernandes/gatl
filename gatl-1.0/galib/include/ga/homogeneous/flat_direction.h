...
#ifndef _GA_HOMOGENEOUS_FLAT_DIRECTION_
#define _GA_HOMOGENEOUS_FLAT_DIRECTION_
...
{
	// Store type.
	typedef typename lcont_return<model,ga::store<model::origin_vector_bitmap>,store>::store_t store_t; 
};
...
flat_direction(const multivector<model,store> &m, const real_t &tolerance _GA_DEFAULT_TOLERANCE)
{
	...
	return lcont( _private::consts<model>::ie0(), m );
}
...
#endif // !_GA_HOMOGENEOUS_FLAT_DIRECTION_
