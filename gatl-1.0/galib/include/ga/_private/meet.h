...
#ifndef _GA_PRIVATE_MEET_
#define _GA_PRIVATE_MEET_
...
/*******************************************************************************************************
 * ga::_private::meet<model,store1,store2> struct definition.
 *******************************************************************************************************/

// The base struct for create the meet of two blades.
...
public:
	...
	multivector<model,meet_store_t> run(const multivector<model,store1> &A, const multivector<model,store2> &B, const int gradeA, const int gradeB, const bool swapped, const real_t &tolerance)
	{
		...
		{
			M = multivector<model,store_none>();
		}
		else
		{
			...
			if (gradeD != (gradeA + gradeB))
			{
				...
				for (index_t i=0; E!=0; ++i)
				{
					...
					if ((E & vector_t::basis_blade( i ).bitmap) != 0)
					{
						...
						// If the projection is not zero, then wedge it to the meet (update M). If the new grade of M is the required grade of the meet then return it.
						...
						// If the rejection is not zero, then remove it from the join (update J). If the new grade of J is the required grade of the join then compute the meet from it.
						if (!ga::is_zero( ri, tolerance ))
						{
							...
							// We're done! Compute meet from join.
							if (JCount == 0)
							{
								ga::assign<false>( M, lcont_em( lcont_em( B, inverse_em( J, tolerance ) ), A ), tolerance );
								break;
							}
						}
