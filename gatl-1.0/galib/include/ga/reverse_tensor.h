...
#ifndef _GA_REVERSE_TENSOR_
#define _GA_REVERSE_TENSOR_
...
/*******************************************************************************************************
 * Reverse operation tensor.
 *******************************************************************************************************/

// Computes the tensor representation of the reverse operation.
...
reverse_tensor()
{
	...
	if (first_time)
	{
		#pragma omp critical (GA_REVERSE_TENSOR)
		{
			if (first_time)
			{
				...
				for (index_t i=0; i!=model::basis_blades_count; ++i)
				{
					...
					const multivector_t &aux = reverse( blade_i );
