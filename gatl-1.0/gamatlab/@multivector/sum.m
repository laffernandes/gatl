...
if isscalar(m) || (dim > n)
    ...
    operation = str2func([model_name '_plus']);
    try
        ...
        r = multivector(zeros([1 prod(dims(2:end))]));
        for i=1:dims(1)
        ...
        switch err.identifier
            case 'MATLAB:UndefinedFunction'
                error('GAToolbox:UndefinedFunction','Geometric algebra model ''%s'' does not implement ''plus'' or it was not compiled.',model_name)
            otherwise
                rethrow(err);
        end
    end
