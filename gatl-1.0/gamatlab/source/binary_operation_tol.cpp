...
mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	if ((nelems1 == nelems2) || (nelems2 == 1))
	{
		...
		plhs[0] = alloc_struct_array( ndims1, dims1, nelems1 );
	}
	else
	{
		const int ndims2 = mxGetNumberOfDimensions( arg2 );
		const int *dims2 = mxGetDimensions( arg2 );
		plhs[0] = alloc_struct_array( ndims2, dims2, nelems2 );
	}
	...
	if (nelems1 == nelems2)
	{
		...
		for (i=0; i<nelems1; ++i)
		{
			...
			to_matlab( result, i, OPERATION( to_multivector( arg1, i ), to_multivector( arg2, i ), tol ) );
	...
	if (nelems2 == 1)
	{
		...
		{
			OMP_LOOP_EXCEPTION_HANDLING_BEGIN
			to_matlab( result, i, OPERATION( to_multivector( arg1, i ), m2, tol ) );
			OMP_LOOP_EXCEPTION_HANDLING_END
		}
	}
	...
	if (nelems1 == 1)
	{
		...
		{
			OMP_LOOP_EXCEPTION_HANDLING_BEGIN
			to_matlab( result, i, OPERATION( m1, to_multivector( arg2, i ), tol ) );
			OMP_LOOP_EXCEPTION_HANDLING_END
		}
	}
